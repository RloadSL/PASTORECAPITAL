...
const validateCode = async ({ email, code }: { email: string, code: number }, context: https.CallableContext) => {
  ...
  try {
    ...
    if (codeDate > new Date(codeDate.setDate(1))) {
      return { status: 401, error: 'code@expired' }
    }

    if (code.toString() == user?.security_code.code.toString()) {
      await setUser(uid, { security_code: { code: 1, created_at: new Date(new Date().toLocaleDateString('es-ES')) }, })
      return { status: 200, error: null }
    }

    return { status: 401, error: 'code@invalid' }
  } catch (error) {
    console.error(error)
    return { status: 500, error }
  }
