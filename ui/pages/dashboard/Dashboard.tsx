...
import toolsIcon from "../../../assets/img/calculator.png";
import webinarsIcon from "../../../assets/img/binoculars.png";
...
const NewsList = () => {
  ...
  useEffect(() => {
    let fetch = true;
    if (!news)
      newsRepository.getNews({ search: "", tickers: "AllTickers", items: 3 }).then((result) => {
  ...
  return !news ? <Loading loading variant="inner-primary" /> : <InfoList list={news} />;
};
...
const LastWebinarRender = () => {
  ...
  return !webinar ? (
    <div className={style.emptyWebinars}>
      <div className={style.emptyWebinars_icon}>
      <Image src={webinarsIcon} alt="Empty webinars" />
      </div>
      <p>
        <FormattedMessage id="page.dashboard.webinars.empty" />
      </p>
    </div>
  ) : (
    <div className={style.lastWebinar}>
      <div className={style.top}>
        <div className={style.lastWebinar_date}>
          <span className={style.lastWebinar_date__day}>{webinar?.date.getDate()}</span>
...
const FlashUpdatesRender = () => {
  ...
  useEffect(() => {
    let fetch = true;
    FlashUpdatesRepositoryInstance.getArticles(userLogged?.userDataToken, userLogged?.wpToken, {
      offset: 1,
      post_status: "public",
    ...
    <InfoColorCard alt={"Imagen de cursos"} backgroundColor="#F2E4FF" iconHref={amasIcon}>
      <div className={style.amasCardText}>
        {ama ? (
          ...
          <p>
            <FormattedMessage id="page.dashboard.amas.empty" />
          </p>
        )}
      </div>
    </InfoColorCard>
  ...
  return (
    <div className={style.dashboard}>
      <div className={style.dashboard_grid}>
        ...
        <div className={`${style.dashboard_grid_item} ${style.item__span5} ${style.flashUpdates}`}>
          <div className={style.item_header}>
            <span>
            ...
            <Link href={"/tools"}>
              <a>
                <InfoColorCard
                  backgroundColor="#DBDEFF"
                  iconHref={toolsIcon}
                  alt={"Calculadora de riesgo"}
                >
                  <p className={style.colorCardText}>
                    <FormattedMessage
                      id="page.dashboard.calculator.text"
                      values={{
                        b: (children) => <span>{children}</span>
                      }}
                    />
