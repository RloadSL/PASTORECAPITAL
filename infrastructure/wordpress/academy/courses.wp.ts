...
export const createWpCourse = async (course:PAGES, wpToken: string) => {
  const primaryCat = await getCategoryAcademy('courses', wpToken)
  
  const cat = await HTTP.post(WP_API_CATEGORY, {name: course.title}, {Authorization: `Bearer ${wpToken}`})
  if(cat.data?.id){
    const resPage = await HTTP.post(WP_API_PAGES, {...course, status:'private', categories: [cat.data.id, course.level.id, primaryCat]}, {Authorization: `Bearer ${wpToken}`})
    if(resPage.data.id){
...
const getCategoryAcademy = async (target:'courses' | 'tutorials', wpToken:string):Promise<number> => {
  ...
  if(academyCat.length === 0){
    const cat = await HTTP.post(WP_API_CATEGORY, {name: 'Academy '+target, slug: 'academy_'+target}, {Authorization: `Bearer ${wpToken}`})
    return cat.data.id
  }else{
